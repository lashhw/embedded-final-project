    ...
    def analysis(self):
        ...
        try:
            ...
            conn = http.client.HTTPSConnection(
                "nycu-embedded-cv.cognitiveservices.azure.com"
            )
            conn.request(
                "POST",
            ...
            result["objects"] = [
                sorted(obj["tags"], key=lambda x: x["confidence"], reverse=True)[0][
                    "name"
                ]
                for obj in j["objectsResult"]["values"]
            ]
            result["text"] = j["readResult"]["content"]
            result["people_count"] = sum(
                1
                for x in j["peopleResult"]["values"]
                if x["confidence"] > self.PEOPLE_CONFIDENCE_THRESHOLD
            )
